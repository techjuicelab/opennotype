...
def send_request(case, model, api_key):
    ...
    try:
        ...
        if not 200 <= status < 300:
            ...
            return {"http_status": status, "error": "redirect_rejected" if 300 <= status < 400 else "http_error"}
        body = response.read(MAX_RESPONSE_BYTES + 1)
        if len(body) > MAX_RESPONSE_BYTES:
            return {"http_status": status, "error": "response_too_large"}
        try:
            obj = json.loads(body)
        except (ValueError, UnicodeDecodeError):
            return {"http_status": status, "error": "invalid_response_json"}
        if not isinstance(obj, dict):
            return {"http_status": status, "error": "invalid_response_json"}
        return {"http_status": status, "object": obj}
    ...
    try:
        ...
        if args.execute:
            ...
            execute_plan(plan, before, after, os.environ.get("GROQ_API_KEY"),
                         save=lambda value: write_report(value, args.output))
            write_report(plan, args.output)
