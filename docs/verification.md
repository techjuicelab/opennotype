...
The default `ProcessingPrompt.swift` and `DictationCleanupInstructions.swift` are restored exactly to Codex `233fd1f`. In the same 9 synthetic cases, Codex passed meaning preservation on 9/9 and cleanup on 7/9; the revised combined candidate passed meaning on 9/9 and cleanup on 6/9. Neither had a major information loss in those cases. Among the 5 cases completed by all three original variants, cleanup passed on 4/5 for Codex, 2/5 for Claude `ff9082e`, and 2/5 for the first combined candidate. These are separate matched comparisons, not a pooled success rate or evidence of general superiority.

The update adopts history UI, contrast fixtures and evaluation tooling; it does not ship the candidate prompt changes. There were 42 API attempts in this comparison work: 41 normal responses and one earlier HTTP 429. Inputs were synthetic text. Candidate source snapshots and exact request exports are retained as experimental evidence under [integration evidence](reviews/2026-09-12/integration-evidence/); the [live evaluation report](reviews/2026-09-12/faithful-cleanup-live-comparison.md) records version mapping and results. The [implementation report](reviews/2026-09-12/typeless-integration.md) describes the adopted changes and boundaries.

History provides original/result comparison and separate copying, plus manual text reprocessing with current provider/model/profile/dictionary and a visible cost notice. Reprocessing leaves the original record intact, uses the existing usage callback and never automatically inserts the preview into another app. Missing historical selected text blocks voice-edit reprocessing. Cancellation, deletion, late responses and rejected recording starts have regression coverage.

Final Swift suite: **300 tests discovered, 298 passed, 2 opt-in model tests skipped, 0 failures**. The comparison tools have **45 passing offline tests**. Adopted dictation instructions have a maximum of **9,112 UTF-8 bytes across 20 controlled profiles**, below the 10,400-byte cap; bytes are not billable tokens. The two adopted prompt source files match Codex `233fd1f`, and all 10 exported comparison requests match the evaluated Codex export exactly. The final **0.1.9 (10)** development app builds and passes strict codesign verification. Native light/dark history preview checks use synthetic data and no real keys, recording or network. Natural speech, actual Typeless outputs and the full target-app matrix remain separate checks.
...
The [38 semantic fixtures](fixtures/dictation-quality.json) remain human-authored specifications. `DisfluencyPolicyTests` now loads that file and checks specification integrity only — that every case composes a prompt, that its spoken text reaches the JSON payload byte-identical, that instructions depend on nothing but mode, kind and tone, and that each declared `disfluency_tokens` count matches its own `stt_input` and `expected_text`. It calls no model and measures no model output. Natural voice accuracy, unspoken-example leakage, live API compatibility, translation naturalness, and same-audio model comparisons are pending. No paid API or personal audio was used for this revision's automated verification.
