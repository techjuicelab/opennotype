...
@main struct Benchmark {
    static func main() async throws {
        ...
        let directory = FileManager.default.temporaryDirectory.resolvingSymlinksInPath()
            .appendingPathComponent("OpenNoTypeStorageBenchmark-\(count)-\(UUID().uuidString)")
        ...
        for iteration in 1...3 {
            ...
            print(String(format: "failures=%d snapshot%d=%.3fs result=%d", count, iteration, ProcessInfo.processInfo.systemUptime - started, result.failedRecordings.count))
        }
    }
}
